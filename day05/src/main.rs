...
fn main() {
    ...
    let p1: String = top_crates(
      instructions.clone().fold(initial_state.clone(), process_instruction_p1)
    );

    let p2: String = top_crates(
      instructions.fold(initial_state, process_instruction_p2)
    );
...
fn process_instruction_p1(mut stacks: Vec<VecDeque<char>>, instruction: (usize, usize, usize)) -> Vec<VecDeque<char>> {
...
fn process_instruction_p2(mut stacks: Vec<VecDeque<char>>, instruction: (usize, usize, usize)) -> Vec<VecDeque<char>> {
  let (num_crates, from_stack, to_stack) = instruction;
